...
import com.example.shopinglist_app.domain.interactor.AddShopItemUseCase
import com.example.shopinglist_app.domain.interactor.GetShopItemUseCase
import com.example.shopinglist_app.domain.interactor.RefactorShopItemUseCase
...
class ShopItemViewModel:ViewModel() {
    ...
    fun refactorShopItem(inputCount: String?, inputName: String?){
        val name = parseName(inputName)
        val count = parseCount(inputCount)
        val fieldsValid = validateInput(name,count)
        if (fieldsValid){
            _shopItem.value?.let {
                ...
                refactorShopItemUseCase.refactorShopItem(item)
                finishWork()
            }
        }
    }
